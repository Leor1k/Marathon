...
namespace FromExelToInser
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private void position_bt_Click(object sender, RoutedEventArgs e)
        {
            ExcelPackage.LicenseContext = LicenseContext.NonCommercial;
            OpenFileDialog openFileDialog = new OpenFileDialog
            {
            ...
            if (openFileDialog.ShowDialog() == true)
            {
                ...
                using (ExcelPackage package = new ExcelPackage(file))
                {
                    ...
                    int colCount = worksheet.Dimension.Columns;
                    //rowCount = 10;
                    string pathIsert = "INSERT INTO [dbo].[Position] ([PositionId], [PositionName], [PositionDescription], [PayPeriod], [PayRate]) VALUES";
                    for (int row = 2; row <= rowCount; row++)
                    {
                        for (int col = 1; col <= colCount; col++)
                        {
                    ...
                    string desktopPath = Environment.GetFolderPath(Environment.SpecialFolder.Desktop);
                    string path = Path.Combine(desktopPath, "InsertPosition.sql");
                    using (FileStream fs = File.Create(path))
                    {
                        byte[] info = new UTF8Encoding(true).GetBytes(pathIsert);
                        // Добавляем информацию в файл.
        ...
        private void staff_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (openFileDialog.ShowDialog() == true)
            {
                ...
                using (ExcelPackage package = new ExcelPackage(file))
                {
                    ...
                    int colCount = worksheet.Dimension.Columns;
                    //rowCount = 10;
                    string pathIsert = "INSERT INTO [dbo].[Staff] ([StaffId], [FirstName], [LastName], [DateOfBirth], [Gender], [PositionId], [Email]) VALUES ";
                    for (int row = 2; row <= rowCount; row++)
                    {
                        for (int col = 1; col <= colCount; col++)
                        {
                            ...
                            switch (col)
                            {
                                ...
                                case 3:
                                    ...
                                    break;
                                case 4:
                                    pathIsert += $"N'{worksheet.Cells[row, col].Text}', ";
                                    break;
                                case 5:
                                    pathIsert += $"N'{worksheet.Cells[row, col].Value?.ToString()}', ";
                                    break;
                    ...
                    string desktopPath = Environment.GetFolderPath(Environment.SpecialFolder.Desktop);
                    string path = Path.Combine(desktopPath, "InsertStaff.sql");
                    using (FileStream fs = File.Create(path))
                    {
                        byte[] info = new UTF8Encoding(true).GetBytes(pathIsert);
                        // Добавляем информацию в файл.
